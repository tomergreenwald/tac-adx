...
public class GreedyLuckyAdNetwork extends Agent {
	...
	private final Logger log = Logger
			.getLogger(GreedyLuckyAdNetwork.class.getName());
	...
	private void handleAdNetworkDailyNotification(
		...
		ucsModel.ucsUpdate(notificationMessage.getServiceLevel(), notificationMessage.getPrice(), activeCampaigns());		
	...
	private void handleSimulationStatus(SimulationStatus simulationStatus) {
		...
		System.out.println("Day " + day + ": Ucs bid is " + ucsModel.getBid());
		/* Note: Campaign bid is in millis */
		AdNetBidMessage bids = new AdNetBidMessage(ucsModel.getBid(), pendingCampaign.id, cmpBidMillis.longValue());
	...
	protected void sendBidAndAds() {
		...
		for (CampaignData campaign : myCampaigns.values()) {
			...
			if ((dayBiddingFor >= campaign.dayStart)
				...
				System.out.println("Day " + day + ": Updated Bid Bundle entries " +
				 "for Campaign id " + campaign.id);
			}
		}
	...
	private class UcsModel {
		...
		/*
		 * The current bid and targetted percentile for the user classification service
		 */
		...
		// latest reported level level and cost, to be applicable during the following simulation day
		...
		public UcsModel() {
			...
			// logistic regression initial values and constants for prob(toplevel | bid).
			ucsAlpha = -10.0;
			ucsBeta = 10.0;
		...
		public void ucsUpdate(double level, double cost, boolean bidHigh)  {
			...
			// apply logistic regression update for pr(Top|bid) parameters, using current ucsBidPercentile and ucsBid
			...
			ucsGamma += ucsLearningRate*(cost - (ucsGamma + ucsDelta*level));
			ucsDelta += ucsLearningRate*(cost - (ucsGamma + ucsDelta*level))*level;
		}
	...
	private class CampaignData {
		...
		public String toString() {
			return "Campaign ID " + id + ": " + "day " + dayStart + " to "
					+ dayEnd + " " + MarketSegment.names(targetSegment) + ", reach: " + reachImps
