...
public class DefaultAdxUserManagerBuilder implements
	...
	private static final String ADX_QUERY_MANAGER_DEFAULT = "tau.tac.adx.users.AdxUserQueryManagerBuilder";
	...
	public DefaultAdxUserManager build(ConfigProxy userConfigProxy,
		...
		try {
			...
			AdxUserBehaviorBuilder<AdxUserViewManager> viewBuilder = ConfigProxyUtils
					.createObjectFromProperty(userConfigProxy, ADX_BASE + '.'
							+ VIEW_MANAGER_KEY, VIEW_MANAGER_DEFAULT);
			AdxUserViewManager viewManager = viewBuilder.build(userConfigProxy,
					repository, random);
