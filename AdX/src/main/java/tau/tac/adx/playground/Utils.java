...
public class Utils {
	...
	public static double [] DC(int m, double [][] featureVector, double [] initial_w,
			double [] b1, double [] b2, double gamma, double lambda, double Lambda) throws Exception {
		...
		// Try the given Lambda and a range of alphas.  If we cannot find a valid solution, double Lambda and repeat.
		while (!found_valid_sol) {	
			min_val = Double.MAX_VALUE;
			curr_min_idx = 0;
			
			// For each alpha, try to solve the QP.
			// Sometimes an alpha may not lead to a valid solution.  Continue if that happens.
			// Keep track of the best QP value
			for (int aa = 0; aa < va.length; ++aa) {
				double alpha = va[aa];
				System.out.println("alpha = " + alpha);
				initial_w = estimate_w(m, numFeatures, featureVector, initial_w, b1, b2, alpha, lambda, Lambda);
				...
				while (del_dist > threshold  &&  dc_iter < 10) {
					...
					System.out.println("Iter " + dc_iter + ", euclidean_norm (loop start) = " + euclidean_norm);
					
					w = dca(m, numFeatures, featureVector, w, b1, b2, gamma, lambda, Lambda, qpval, aa);
					System.out.println("qpval = " + qpval[aa] + ", w_dca_norm = " + calculate_euclidean_norm(w));
					...
					// if block is there because we may have u dot xi = 0 for all i
					...
					if (num_pos_u_dot_xi > 0) {
						...
						for (int i = 0; i < m; i++) {
							if (u_dot_xi[i] > 0.0) {
								...
								functions[c] = new VFunction(new Boundary(low, high), new Points(
										a1, a2, a3, a4));
								c++;
							}
						}
						
						double eta = minimize_f_fast_with_rejection(functions, Lambda);
						...
						valid_alpha = false;
						System.out.println("Got a u that does not give positive bids!" + "  alpha = " + alpha);
						break;
					}
					if (valid_alpha)
			...
			// Nothing found with this combination of parameters.  Try increasing Lambda by 2.
			if (!found_valid_sol)
				Lambda *= 2.0;
		}
		...
		System.out.println("Best alpha = " + va[curr_min_idx] + " with value " + min_val);
		System.out.println("w_norm = " + calculate_euclidean_norm(w));
		System.out.println("First " + T + " elements of w: " + Arrays.toString(first_T_w));
	...
	public static double [] dca(int m, int numFeatures, double [][] x, double [] initial_w,
			double [] b1, double [] b2, double gamma, double lambda, double Lambda, double [] qpval, int aa) {
		...
		while (maxDifference > EPSILON  &&  iter < 10) {
			...
			double [] sgv = calculate_subgradient(m, numFeatures, w, x,
					b2, b1, gamma);	
			double [] lp_results = solve_lp(iter, m, numFeatures, x,
					b1, b2, gamma, lambda, Lambda, sgv, qpval, aa);
			
			for (int j = 0; j < numFeatures; ++j) {
				double currDiff = Math.abs(
						w[numFeatures - (j + 1)]
						   - lp_results[lp_results.length - (j + 1)]);
				if (currDiff > maxDifference) {
					maxDifference = currDiff;
				}
				w[numFeatures - (j + 1)] = lp_results[lp_results.length - (j + 1)];
			}
		}
	...
	private static double[] solve_lp(int iter, int m, int numFeatures, double [][] x,
			double [] b1, double [] b2, double gamma, double lambda, double Lambda, double [] sgv,
			double [] qpval, int aa) {
		...
		try {
			...
			IloLPMatrix lp = PopulateQP(cplex,
					m, numFeatures, x, b1, b2, gamma, lambda, Lambda, sgv);
			if (cplex.solve()) {
	...
	private static IloLPMatrix PopulateQP(IloMPModeler model,
			int m, int numFeatures, double [][] x,
			double [] b1, double [] b2, double gamma, double lambda,
			double Lambda, double [] sgv)
			throws IloException {		
		...
		IloNumVar[] s = model.numVarArray(model.columnArray(lpMatrix, m), lb, ub);
		...
		IloNumVar[] w = model.numVarArray(model.columnArray(lpMatrix, numFeatures), lb_w, ub_w);
		...
		for (int i = 0; i < m; ++i) {			
			...
			for (int j = 0; j < numFeatures; ++j) {
				inter[j] = model.prod(-1.0,
						model.prod(x[i][j], w[j]));
			}
			inter[numFeatures] = model.sum(model.prod(s[i], gamma), (1.0 + gamma) * b1[i]);
			model.addGe(model.sum(inter), 0.0);
		}
	 ...
	 */
	public static double [] estimate_w(int m, int numFeatures, double [][] x, double [] initial_w,
			double [] b1, double [] b2, double alpha, double lambda, double Lambda) {
		...
		while (maxDifference > EPSILON  &&  iter < 10) {
			...
			double [] interim_w = solve_surrogate(iter, m, numFeatures, x,
					b1, b2, alpha, lambda, Lambda);
	...
	private static double[] solve_surrogate(int iter, int m, int numFeatures, double [][] x,
			double [] b1, double [] b2, double alpha, double lambda, double Lambda) {
		double [] cplex_result = new double [numFeatures + m];
		try {
			IloCplex cplex = new IloCplex();
			//cplex.setParam(IloCplex.IntParam.Threads, 1);
			cplex.setOut(null);	// Disable console output
			IloLPMatrix lp = PopulateSurrogateQP(cplex,
					m, numFeatures, x, b1, b2, alpha, lambda, Lambda);
			if (cplex.solve()) {
				cplex_result = cplex.getValues(lp);
			}
			cplex.endModel();
	...
	private static IloLPMatrix PopulateSurrogateQP(IloMPModeler model,
			int m, int numFeatures, double [][] x,
			double [] b1, double [] b2, double alpha, double lambda,
			double Lambda)
			throws IloException {		
		IloLPMatrix lpMatrix = model.addLPMatrix();
		...
		for (int i = 0; i < m; ++i) {			
			...
			double Q = ((1.0 - alpha) * b1[i] + alpha * b2[i]) / (alpha * (b1[i] - b2[i]));
