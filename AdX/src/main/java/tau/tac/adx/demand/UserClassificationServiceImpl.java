...
public class UserClassificationServiceImpl implements UserClassificationService {
	...
	private Map<String,UserClassificationServiceAdNetData> advertisersData;

	@Override
	public void updateAdvertiserBid(String advertiser, double ucsBid, int day) {
		UserClassificationServiceAdNetData advData = advertisersData.get(advertiser);
		if (advData == null) 
	...
	public void auction(int day) {		
		...
		if (advCount > 0) {			
			...
			for (int j = 0; j < advCount; j++ ) {
				UserClassificationServiceAdNetData advData = advertisersData.get(advNames[indices[j]]);
