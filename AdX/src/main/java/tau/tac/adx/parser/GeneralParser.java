			...
			if (content instanceof BankStatus) {
				if (day > 1) {
					...
					if (day > 2) { /* verify reposted balance for day-2 */
						...
						double expectedBalance = receiverData.daysData[verifiedDay].accumulatedRevenue -
						(receiverData.daysData[verifiedDay].adxAccumulatedCosts + receiverData.daysData[verifiedDay].ucsAccumulatedCosts);
