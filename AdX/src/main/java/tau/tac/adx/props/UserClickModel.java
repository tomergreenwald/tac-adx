...
/**
 * UserClickModel the parameters that generate user viewing behavior. These
 * parameters include (for each query class):
 * <ul>
 ...
 *      Documentation</a>
 * @param <T>
 *            User click model type.
 * @author Patrick Jordan
 */
public class UserClickModel<T> extends AbstractTransportable {
	/**
	 * The advertiser effects (see <a
	 * href="http://aa.tradingagents.org/documentation">TAC Documentation</a>).
	 */
	private double[][] advertiserEffects;
	/**
	 * The continuation probabilities (see <a
	 * href="http://aa.tradingagents.org/documentation">TAC Documentation</a>).
	 */
	...
	public UserClickModel() {
		this(new TacQuery[0], new String[0]);
	}

	/**
	 * Creates a user click model initialized with the {@link Query queries} and
	 * advertisers.
	 * 
	 * @param queries
	 *            the initial queries
	 * @param advertisers
	 *            the initial advertisers.
	 */
	public UserClickModel(final TacQuery<T>[] queries,
			final String[] advertisers) {
		if (queries == null) {
	 ...
	 * 
	 * @param index
	 *            the index into the advertiser set.
	...
	/**
	 * Returns the index for an advertiser and <code>-1</code> if the advertiser
	 * is not in the model.
	 * 
	 * @param advertiser
	 *            the advertiser.
	 * @return the index for an advertiser and <code>-1</code> if the advertiser
	 *         is not in the model.
	 */
	 ...
	 * 
	 * @param index
	 *            the index into the query set.
	 * @return the query at the index.
	 */
	public final TacQuery<T> query(final int index) {
		return queries[index];
	}

	/**
	 * Returns the index for a query and <code>-1</code> if the query is not in
	 * the model.
	 * 
	 * @param query
	 *            the query.
	 * @return the index for a query and <code>-1</code> if the query is not in
	 *         the model.
	 */
	public final int queryIndex(final TacQuery<T> query) {
	 ...
	 *      Documentation</a>
	 */
	public final void setContinuationProbability(final int queryIndex,
	...
	/**
	 * Returns the advertiser effect for the query and advertiser at the
	 * indices.
	 * 
	 * @param queryIndex
	 *            the query index.
	 * @param advertiserIndex
	 *            the advertiser index.
	 * @return the advertiser effect for the query and advertiser at the
	 *         indices.
	 * @see <a href="http://aa.tradingagents.org/documentation">TAC
	 *      Documentation</a>
	 */
	public final double getAdvertiserEffect(final int queryIndex,
			final int advertiserIndex) {
	 ...
	 *            the advertiser index.
	 * @param effect
	 *            the advertiser effect.
	 * @see <a href="http://aa.tradingagents.org/documentation">TAC
	 *      Documentation</a>
	 */
	public final void setAdvertiserEffect(final int queryIndex,
			final int advertiserIndex, final double effect) {
		lockCheck();
		advertiserEffects[queryIndex][advertiserIndex] = effect;
	}

	/**
	 * Reads the queries, advertisers, advertiser effects, and continuation
	 * probabilities from the reader.
	 * 
	 * @param reader
	 *            the reader to read data from.
	 * @throws ParseException
	 *             if an exception occurs reading the parameters.
	 */
	@Override
	protected final void readWithLock(final TransportReader reader)
		...
		queries = queryList.toArray(new TacQuery[0]);
	...
	/**
	 * Writes the queries, advertisers, advertiser effects, and continuation
	 * probabilities to the writer.
	 * 
	 * @param writer
	...
	protected final void writeWithLock(final TransportWriter writer) {
		for (TacQuery<T> query : queries) {
			writer.write(query);
		}

		for (String advertiser : advertisers) {
			writer.node("advertiser").attr("name", advertiser)
					.endNode("advertiser");
		}
		...
		for (int queryIndex = 0; queryIndex < queries.length; queryIndex++) {
			for (int advertiserIndex = 0; advertiserIndex < advertisers.length; advertiserIndex++) {
				writer.node("advertiserEffect")
						.attr("queryIndex", queryIndex)
